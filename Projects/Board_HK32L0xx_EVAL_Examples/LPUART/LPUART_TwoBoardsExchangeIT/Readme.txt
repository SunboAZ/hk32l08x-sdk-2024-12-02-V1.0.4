...
******************************例程详细描述*****************************
功能描述:
	本示例为UART3使用中断发送数据和接收数据的功能。BPS为9600。
	上电后,先按下BOARD2按键1，BOARD2等待接收数据。
	再按下BOARD1的按键1会向BOARD2发送数据 0x55,0x01
	并等待接收数据。
	BOARD2收到后发送0x55,0x01，并比较接收第一个数据是否为0x55,一致则亮起LED1,LED2
	BOARD1收到数据后并比较接收第一个数据是否为0x55，一致则亮起LED1,LED2。
