...
int main(void)
{
    ...
    */
    ...
    /* Start to  continue */
    printf("Wait for User push-button press to start OPAMP PGA mode ...\r\n");
    ...
    while (1)
    {
        ...
        {
            uADC_EOCFlag = 0;
            printf(" After OPAMP1 PGA Gain 2,The measured voltage:%f\r\n", ADCVal);
...
static void OPAMP_Function_Handle(void)
{
    /* ADC configuration: Channel 2 (PA5) is internally connected to OPAMP1 output,
     End Of Conversion (EOC) interrupt is enabled */
    ADC_Config();

    /* OPAMP2 configuration: OPAMP1 in internal follower mode, OPAMP1 output is
     internally connected to ADC Channel 2 */
    OPAMP_Config();
...
static void OPAMP_Config(void)
{
    ...
    OPAMP_InitStructure.InputInverting = OPAMP_MODE_PGA;
    ...
    OPAMP_SetPGAGain(OPA1, OPAMP_PGA_GAIN_2);
