...
int main(void)
{
    ...
    /* Wait Until KEY1 BUTTON is pressed */
...
static void RTC_Config(void)
{
    ...
    /* Calendar Configuration */
    RTC_InitStructure.RTC_AsynchPrediv = 127;
    RTC_InitStructure.RTC_SynchPrediv = 255; /* (32.768KHz / 128) - 1 = 255*/
    RTC_InitStructure.RTC_HourFormat = RTC_HourFormat_24;
    RTC_Init(&RTC_InitStructure);
...
static uint32_t GetLSIFrequency(void)
{
    ...
    /* check PCLK1 prescale: =1: TIMCLK = PCLK1; else TIMCLK = 2*PCLK1 */
    if ((RCC->CFGR &  RCC_CFGR_PPRE) & (0x4 << 8))
    {
        TimerClkScale = 2;
    }
    else
    ...
    return ((RCC_ClockFreq.PCLK_Frequency / PeriodValue) * 8 * TimerClkScale);
}
